...
test.beforeEach(async ({ page }) => {
  ...
  searchResultsPage = new SearchResultPage(page);
...
test('Add product to cart test @master @regression', async ({ page }) => {
  ...
  expect(await searchResultsPage.isSearchResultPageExist()).toBeTruthy();
  ...
  if (await searchResultsPage.isProductExist(productName)) {
    ...
    //await searchResultsPage.searchProduct(productName);
