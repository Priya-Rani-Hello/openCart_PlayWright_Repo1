...
- Name: LoginDataDriven.spec.ts >> Login with Json data:InValid Logon @datadriven
- Location: tests\LoginDataDriven.spec.ts:10:5
        ...
        - generic [ref=e87]:
          ...
          - generic [ref=e91]:
            ...
            - generic [ref=e95]:
              ...
              - textbox "Password" [ref=e97]: x@abc.comtest@123
...
```ts
  ...
  7  | const jsonpath="testdata/logindata.json";
  8  | const jsonTestData=DataProvider.getTestDataFromJson(jsonpath);
  9  | for( const data of jsonTestData){
  10 | test(`Login with Json data:${data.testName} @datadriven`,async({page})=>{
  ...
  15 | await homePage.clickLogIn();
  16 | const loginPage=new LoginPage(page);
  17 | await loginPage.login(data.email,data.password);
> 18 | if(data.expect.toLowerCase==='success'){
     |                ^ TypeError: Cannot read properties of undefined (reading 'toLowerCase')
  19 | const myAccountPage=new MyAccountPage(page);
  20 | const isLoggedIn=await myAccountPage.isMyAccountPageExist();
  21 | expect(isLoggedIn).toBeTruthy();
  22 | }
  23 | else{
  24 |     const errorMessage=await loginPage.getLoginErrorMessage();
  25 |     expect(errorMessage).toBe('Warning: No match for E-Mail Address and/or Password.');
  26 | }
  27 | })
  28 | 
  29 | 
