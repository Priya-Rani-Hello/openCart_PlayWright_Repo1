...
```ts
  ...
  30 | const isMyOrder= await myAccountPage.isMyOrderHeadingExist();
> 31 | expect(isMyOrder).toBe(true);
     |                   ^ Error: expect(received).toBe(expected) // Object.is equality
  32 | //alternatively 
  33 | //await loginPage.login(config.email,config.password);
  34 | //const isLoggedIn= await myAccountPage.isMyAccountPageExist();
  35 | //expect(isLoggedIn).toBe(true);
  36 | //await myAccountPage.clickLogOut();
  37 | })
```