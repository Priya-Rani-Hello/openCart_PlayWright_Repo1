...
```ts
  ...
  20 |     const productNameValue=config.productName;
  21 |     homePage.enterProductName(productNameValue);
  22 |     homePage.clickSearch();
  23 | expect(await searchResultPage.isSearchResultPageExist()).toBeTruthy();
  24 | //const isProductFound = await searchResultPage.isProductExist(productNameValue);
  25 |   //expect(isProductFound).toBeTruthy();
> 26 |   expect(await searchResultPage.isProductExist(productNameValue)).toBeTruthy();
     |                                                                   ^ Error: expect(received).toBeTruthy()
  27 | });
```